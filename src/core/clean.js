...
async function cleanupSingle(entry) {
  ...
  const targetDir =
    entry.targetDir || path.join(os.homedir(), ".config", appName);
  ...
  if (process.env.NVIM_APPNAME === appName) {
    const ok = await confirm({
      message: chalk.redBright(
        `⚠️ '${appName}' is currently active via NVIM_APPNAME. Remove anyway?`,
      ),
      default: false,
    });
    if (!ok) {
      console.log(
        chalk.gray(`⏩ Skipping cleanup for active config '${appName}'.`),
      );
      return { skipped: true };
    }
  }
  ...
  try {
    ...
    console.log(
      chalk.bgYellow.black.bold("\n [CLEANUP STARTED] ") +
        chalk.cyanBright(" 🧹 FkNeo CLI - Clean Prebuilt Setups\n"),
    );
    ...
    if (!meta.length) {
      console.log(
        chalk.gray("No prebuilt configurations detected in meta.json.\n"),
      );
      return;
    }
