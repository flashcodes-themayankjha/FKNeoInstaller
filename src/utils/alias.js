...
export function addShellAlias(aliasName, nvimAppName, isMain = false) {
  ...
  if (!fs.existsSync(rcFile)) {
    console.log(
      chalk.yellow(`⚠️ Shell rc not found at ${rcFile}, skipping alias write.`),
    );
    return;
  }
  ...
  try {
    execSync(`${process.env.SHELL} -lc "source ${rcFile}"`, {
      stdio: "ignore",
    });
    ...
    console.log(
      chalk.cyan(
        `ℹ️ Run 'source ${rcFile}' or restart your terminal to apply changes.`,
      ),
    );
  }
}
